...
def compute(N,filename,method,sup_or_jux=None,diabete='Non',humOrrat = 'human',sup_or_multi = 'superficial',inhibition=None):
    ...
    for i in range(N-1):
        ...
        # solving the system
        if method == 'Newton':
            sol = Newton.newton(equations.conservation_eqs,x,i,cell[i].segment,cell[i].humOrrat,cell[i].sex,cell[i].type,cell[i].diabete,cell[i].inhib)
        if method == 'Broyden':
